...
def clean_text(text):
    ...
    text = re.sub(r"@\w+", "[USER]", text) # Keep track that a specific user was targeted
    text = re.sub(r"#", "", text)          # Leave the hashtag word itself intact
    text = re.sub(r"[^a-zA-Z\s\[\]]", "", text) # Allow brackets for our token
